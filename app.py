...
app.config['SECRET_KEY'] = os.environ.get('SECRET_KEY', "cwkey")
...
def remove_recipe(recipe_id):
	"""Remove recipe from user's collections on recipe page"""
...
def edit_recipe(recipe_id):
	"""Edit recipe's rating, notes, collection and completion"""
...
def show_collection_in_details(collection_id):
	...
	if not g.user:
		...
		return render_template('collections/show_details.html', collection=collection, collections=collections, recipes=recipes)
...
def delete_collection(collection_id):
	...
	if not g.user:
		...
		for recipe in collection.saved_recipes:
			if len(recipe.collections) == 1 and recipe.collections[0].id == collection_id:
				
				saved_recipe = SavedRecipe.query.filter(SavedRecipe.user_id == user_id, SavedRecipe.recipe_id == recipe.recipe_id).first()
				db.session.delete(saved_recipe)

				flash(f'Please note that this collection contains a recipe that is only saved in the collection you are deleting. This recipe is removed from your collection', 'info')
...
@app.route('/collections/recipe/remove/<int:recipe_id>/<int:collection_id>', methods=['POST'])
def remove_recipe_from_collections_page(recipe_id, collection_id):
	"""Remove recipe from user's collection on collection page"""
	...
	if not g.user:
		...
		user_id = g.user.id
		saved_recipe = SavedRecipe.query.filter(SavedRecipe.user_id == user_id, SavedRecipe.recipe_id == recipe_id).first()

		saved_recipe.collections = [collection for collection in saved_recipe.collections if collection.id != collection_id]

		
		if len(saved_recipe.collections) > 0:
			flash(f'This recipe is successfully removed from this collection.', 'success')
			# flash(f'To remove this recipe from other collections, edit and uncheck the unwanted collections.  You can also remove the recipe from all collections on the recipe page.', 'info')

		else:
			db.session.delete(saved_recipe)
			flash(f'This recipe is successfully removed. ', 'success')

		db.session.commit()

		return redirect(f'/collections/show/{collection_id}')


@app.route('/collections/recipe/edit/<int:recipe_id>/<int:collection_id>', methods=['POST'])
def edit_recipe_collections_from_collection_page(recipe_id, collection_id):
	"""Edit recipe's collections on collection page"""
